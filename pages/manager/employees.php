                    ...
                    <div class=" layout-top-spacing">
                        <nav class="breadcrumb-style-five  mb-3" aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a><?php echo $_SESSION['department'] ?> Department</a></li>
                                        ...
                                        <tbody id="accountsTable">
                                            ...
                                            if (isset($filter)) {
                                                if ($filter === "NotEvaluated") {
                                                    ...
                                                    WHERE (archived != 3 AND user_level != 0 AND user_level = 3 AND department = '$d') 
                                                    AND (employee_id != $u AND active = 1)
                                                    ...
                                                    $statement = "ev.evaluator_hr IS NOT NULL AND ev.evaluator_manager IS NULL 
                                                    AND (acc.employee_id != $u AND acc.active = 1) AND (acc.user_level = 3 AND acc.department = '$d')";
                                                }
                                            } else {
                                                $sql = "SELECT * FROM accounts
                                            ...
                                            if ($result && $result->num_rows > 0) {
                                                while ($accounts = $result->fetch_assoc()) {
                                                    ...
                                                    try {
                                                        // Get the for_eval date from database
                                                        if (!empty($accounts['for_eval'])) {
                                                            ...
                                                            // If for_eval is empty, calculate it (fallback only)
                                                    ...
                                                    if ($evalExists) {
                                                        ...
                                                        $buttonClass = 'btn-muted disabled';
                                                        $disabled = 'disabled="disabled"';
                                                        $buttonText = 'HR Form Required';
                                                    }

                                                    $html .= '<tr>';
                                                    $html .= '<td>' . htmlspecialchars($accounts['employee_id']) . '</td>';
                                                    $html .= '<td class="text-center"><span><img src="../../api/' . htmlspecialchars(string: $accounts['img']) . '" class="profile-img rounded-circle" alt="avatar"></span></td>';
